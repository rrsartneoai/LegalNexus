...
export function useSearchLawFirms(initialParams: SearchParams = {}): UseSearchLawFirmsResult {
  ...
  const search = useCallback(async (params: SearchParams) => {
    ...
    try {
      ...
      const errorMessage = err instanceof LawFirmAPIError ? err.message : "Wystąpił błąd podczas wyszukiwania"
...
export function useLawFirm(lawFirmId: string): UseLawFirmResult {
  ...
  const fetchLawFirm = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof LawFirmAPIError ? err.message : "Wystąpił błąd podczas pobierania danych"
      setError(errorMessage)
      setLawFirm(null)
    } finally {
      setLoading(false)
...
export function useCreateLawFirm() {
  ...
  const createLawFirm = useCallback(async (data: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof LawFirmAPIError ? err.message : "Wystąpił błąd podczas tworzenia kancelarii"
      setError(errorMessage)
      throw err
    } finally {
      setLoading(false)
