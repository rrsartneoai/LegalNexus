...
export class LawFirmAPIClient {
  ...
  constructor(
    baseUrl: string = process.env.NEXT_PUBLIC_API_BASE_URL || "https://api.legalnexus.pl",
    apiKey?: string,
    timeout = 30000,
    retryAttempts = 3,
  ) {
  ...
  private async makeRequest<T>(method: string, endpoint: string, params?: Record<string, any>, data?: any): Promise<T> {
    ...
    for (let attempt = 0; attempt < this.retryAttempts; attempt++) {
      try {
        ...
        if (attempt === this.retryAttempts - 1) {
          throw new LawFirmAPIError(error instanceof Error ? error.message : "Network error")
        }
  ...
  async createLawFirm(data: LawFirmCreate): Promise<JSONAPIResponse> {
    ...
    return this.makeRequest<JSONAPIResponse>("POST", "/law-firms", undefined, data)
  }
  ...
  async searchLawFirms(params: SearchParams = {}): Promise<JSONAPIResponse> {
    const searchParams = {
      ...
      ...(params.specializations && { specializations: params.specializations }),
    }

    return this.makeRequest<JSONAPIResponse>("GET", "/law-firms", searchParams)
  }

  async updateLawFirm(lawFirmId: string, data: LawFirmUpdate): Promise<JSONAPIResponse> {
    console.log(`Updating law firm: ${lawFirmId}`)
    return this.makeRequest<JSONAPIResponse>("PUT", `/law-firms/${lawFirmId}`, undefined, data)
  }
  ...
  async createLawyer(lawFirmId: string, data: any): Promise<JSONAPIResponse> {
    return this.makeRequest<JSONAPIResponse>("POST", `/law-firms/${lawFirmId}/lawyers`, undefined, data)
  }
}
