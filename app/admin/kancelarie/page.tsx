...
export default function AdminKancelariePage() {
  ...
  const handleToggleActive = async (firmId: string, isActive: boolean) => {
    try {
      ...
      setLawFirms((firms) => firms.map((firm) => (firm.id === firmId ? { ...firm, is_active: isActive } : firm)))
    } catch (error) {
  ...
  const handleToggleVerified = async (firmId: string, isVerified: boolean) => {
    try {
      ...
      setLawFirms((firms) => firms.map((firm) => (firm.id === firmId ? { ...firm, is_verified: isVerified } : firm)))
    } catch (error) {
      console.error("Failed to update law firm:", error)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Zarządzaj kancelariami prawnymi w systemie</p>
        </div>
        <Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Lista wszystkich kancelarii prawnych w systemie</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                ...
                <TableRow key={firm.id}>
                  <TableCell>
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-500">NIP: {firm.tax_number}</div>
                      </div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm">{firm.is_verified ? "Zweryfikowana" : "Niezweryfikowana"}</span>
                    </div>
                  </TableCell>
                  <TableCell>{new Date(firm.created_at).toLocaleDateString("pl-PL")}</TableCell>
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleToggleActive(firm.id, !firm.is_active)}>
                          {firm.is_active ? "Dezaktywuj" : "Aktywuj"}
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleToggleVerified(firm.id, !firm.is_verified)}>
                          {firm.is_verified ? "Usuń weryfikację" : "Zweryfikuj"}
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleDelete(firm.id)} className="text-red-600">
      ...
      <Dialog open={showDetails} onOpenChange={setShowDetails}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Pełne informacje o kancelarii {selectedFirm?.name}</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{selectedFirm.tax_number}</p>
                </div>
                {selectedFirm.krs_number && (
                  <div>
                    <Label className="text-sm font-medium">KRS</Label>
                    <p className="text-sm text-gray-600">{selectedFirm.krs_number}</p>
                  </div>
                )}
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {selectedFirm.founded_date
                      ? new Date(selectedFirm.founded_date).toLocaleDateString("pl-PL")
                      : "Nie podano"}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {selectedFirm.contact.phone && (
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">{selectedFirm.contact.phone}</span>
                  </div>
                )}
                {selectedFirm.contact.email && (
                  <div className="flex items-center space-x-2">
                    <Mail className="h-4 w-4 text-gray-400" />
                    <span className="text-sm">{selectedFirm.contact.email}</span>
                  </div>
                )}
              ...
              {selectedFirm.description && (
                <div>
                  ...
                  <p className="text-sm text-gray-600">{selectedFirm.description}</p>
                </div>
              )}
              ...
              {selectedFirm.lawyers.length > 0 && (
                <div>
                  <Label className="text-sm font-medium">Prawnicy ({selectedFirm.lawyers.length})</Label>
                  <div className="space-y-2 mt-2">
                    {selectedFirm.lawyers.map((lawyer) => (
                      <div key={lawyer.id} className="flex items-center space-x-3 p-2 bg-gray-50 rounded">
                        ...
                        <div>
                          ...
                          {lawyer.email && <p className="text-xs text-gray-500">{lawyer.email}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <Switch
                      ...
                      onCheckedChange={(checked) => handleToggleActive(selectedFirm.id, checked)}
                    />
                    <span className="text-sm">{selectedFirm.is_active ? "Aktywna" : "Nieaktywna"}</span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <Switch
                      ...
                      onCheckedChange={(checked) => handleToggleVerified(selectedFirm.id, checked)}
                    />
                    <span className="text-sm">{selectedFirm.is_verified ? "Zweryfikowana" : "Niezweryfikowana"}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <Dialog open={showEdit} onOpenChange={setShowEdit}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Zmień informacje o kancelarii {selectedFirm?.name}</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input id="tax_number" defaultValue={selectedFirm.tax_number} />
                </div>
              </div>

              <div>
                <Label htmlFor="description">Opis</Label>
                <Textarea id="description" defaultValue={selectedFirm.description || ""} rows={3} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="phone">Telefon</Label>
                  <Input id="phone" defaultValue={selectedFirm.contact.phone || ""} />
                </div>
                <div>
                  <Label htmlFor="email">Email</Label>
                  <Input id="email" defaultValue={selectedFirm.contact.email || ""} />
                </div>
              </div>

              <div>
                <Label htmlFor="website">Strona internetowa</Label>
                <Input id="website" defaultValue={selectedFirm.contact.website || ""} />
              </div>
            </div>
          )}
